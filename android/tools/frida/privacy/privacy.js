...
function hookMethod(_Class,targetClass, targetMethod, targetArgs, action, messages) {
	
	if (targetMethod == '$init') {//构造方法
        ...
        for (var i = 0; i < overloadCount; i++) {
            ...
            _Class[targetMethod].overloads[i].implementation = function () {
				...
				//console.log(i+"=="+argumentValues);
				if (Array.isArray(targetArgs) && targetArgs.length > 0) {//如果需要有参，过滤无参
					//console.log("进入有参 "+argumentValues);
					if(argumentValues.length<=0){//不注册无参
						return null;
					}
						
					if(targetArgs.every(item => argumentValues.includes(item))){
						var arg = '';
					...
					console.log("进入无参");
					if(argumentValues.length>0){//不注册有参
						return null;
					}
					alertSend(action, messages, "无参数");
				}
				
				
                return temp;//执行完当前的overload方法返回给真实调用的函数
            }
        }
	}
	return true;
    ...
    methodData.forEach(function (methodData) {
        for (var i in methods) {
            if (methods[i].toString().indexOf('.' + methodData['methodName'] + '(') != -1 || methodData['methodName'] == '$init') {
                hookMethod(_Class,targetClass, methodData['methodName'], methodData['args'], methodData['action'], methodData['messages']);
                break;
            }
        }
    });
...
//    ]);
	...
	hook('android.os.Environment', [
        {'methodName': 'getExternalStorageDirectory', 'action': action, 'messages': '写入sdcard'}
    ]);
}
